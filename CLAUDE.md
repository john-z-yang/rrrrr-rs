...
- **sexpr.rs** — Core AST type. `SExpr` enum with variants for Id, Cons, Nil, Bool, Num, Char, Str, Vector. Every node carries a `SourceLoc`. Identifiers carry a `BTreeSet<ScopeId>` for hygienic macro tracking.
...
**main.rs** — REPL loop using rustyline, accumulates multi-line expressions, runs the compile pipeline.
...
- **S-expression representation**: Lists are nested Cons cells (car/cdr). All values carry `SourceLoc` for error reporting through expansions.
